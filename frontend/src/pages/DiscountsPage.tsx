            ...
            <div className="space-y-4">
              {coupons.map((c: any) => {
                ...
                const usagePercent = usageLimit
                  ? Math.round(((c.usedCount || 0) / usageLimit) * 100)
                  : 0;
                return (
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <Badge variant="outline">{getTypeLabel(c.discountType || c.type)}</Badge>
                        </div>
                        <div className="flex items-center gap-4 text-sm text-gray-600 mb-2">
                          <span className="flex items-center gap-1">
                            ...
                            {(c.discountType || c.type) === 'percentage'
                              ...
                              : `${c.discountValue ?? c.value} TND`}
                          </span>
                          {c.validUntil && (
                            <span className="flex items-center gap-1">
                              <Calendar className="w-4 h-4" />
                              Expire le {new Date(c.validUntil).toLocaleDateString('fr-FR')}
                            </span>
                          )}
                        </div>
                        {(c.usageLimit ?? c.maxUses) && (
                          <div className="flex items-center gap-2 text-sm">
                            <span className="text-gray-600">
                              Utilisation: {c.usedCount || 0}/{c.usageLimit ?? c.maxUses}
                            </span>
